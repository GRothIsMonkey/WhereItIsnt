...
bpy.ops.wm.open_mainfile(filepath=str(ROOT/'source/rural_fence_post_01_r03.blend'))
...
for ob in [o for o in scene.objects if o.name.startswith('rural_fence_post_01')]:
    ...
    bm.free()
scene.render.filepath=str(ROOT/'review/13_glb_reimport_neutral.png')
bpy.ops.render.render(write_still=True)
def pixels(path):
    im=bpy.data.images.load(str(path),check_existing=False)
    arr=np.empty(len(im.pixels),dtype=np.float32);im.pixels.foreach_get(arr)
    return arr.reshape(-1,4)[:,:3]
a=pixels(ROOT/'review/01_neutral_three_quarter.png');b=pixels(ROOT/'review/13_glb_reimport_neutral.png')
