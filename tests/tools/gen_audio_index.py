...
W('')
W('')
W('**PHASE 36 — AND IT IS ACTUALLY TRUE NOW.** For two phases it was not. The ceiling was')
W('computed from measurements that could not see past it: first a mono 22.05 kHz downmix')
W('(fixed in 35, unbuildable there), then `volumedetect`, which converts to 16-bit before')
W('it counts and therefore CLAMPS — so a 32-bit float source whose true peak was +8.89 dBFS')
W('reported 0.0 and got 8.9 dB too much gain. That source is `sfx.ui.click`, the interface')
W('click bound to every control in the game, and it shipped with 132 samples pinned flat.')
W('The peak now comes from `astats`, which reads the float domain and does not clamp.')
W('')
W('And a computed gain is only a PREDICTION: sample-rate conversion and lossy encoding both')
W('lift the peak above the highest sample they were given, by up to 1.33 dB measured. The')
W('builder now encodes, MEASURES WHAT CAME OUT, and encodes once more from the original if')
W('it landed over. 27 files were rebuilt; the other 247 were already correct and were left')
W('alone. Verified in a real decoder (`tests/tools/measure_runtime.js`): **0 files clipped**')
W('(was 3), and the two still above the ceiling are over it by 0.30 and 0.03 dB with no')
W('pinned samples — MP3 decoder disagreement, not a build fault.')
W('')
W('Six files carry a DC offset between 0.005 and 0.027. Inaudible, costs under a quarter of')
W('a decibel of headroom, and removing it means filtering files that are otherwise correct.')
W('Measured and left alone rather than churned.')
