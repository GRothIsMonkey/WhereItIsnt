...
npm run budgets            # D1 Phase 3: the visual budgets, and how anything is measured
npm run browser-budgets    # D1 Phase 3, live: a real GLB and the real streamed world
```
...
| `budgets.js` | D1 Phase 3. **The budget numbers are PARSED OUT OF `VISUAL_RULE_BIBLE.md` section 9.1 rather than transcribed**, joined to the model by the bible's own row labels, so a drift in either direction is a failure here. Then: advisory vs blocking is explicit and being UNDER a minimum never blocks; the tolerance is a fraction of the boundary, so one number works for a 500-triangle class and a 30,000-triangle one. What a triangle IS — indexed and non-indexed, groups that partition rather than multiply, an InstancedMesh counted once, shared geometry counted per mesh, hidden and collision-only excluded and attributed, Points and Lines at zero. Materials: shared reuse and *equivalent materials instantiated repeatedly* are different numbers and both are reported; a texture with no image is MISSING and one with no dimensions is UNKNOWN SIZE. Texel density against fixtures with predictable answers (a 1 m quad with a 64² map is exactly 64 px/m), with every unavailable reason tested separately. Every asset class validated four ways — compliant, boundary, clearly over, and excepted. Exceptions must be local, stated and justified; an invalid one excuses nothing, one that is too small excuses nothing, and one that was not needed is itself a finding. All fixtures are synthetic plain objects, which is also how "these modules name no THREE" is proved rather than asserted |
| `browser-budgets.js` | D1 Phase 3 **in Chromium, over HTTP**. Both modules load in the shipped page. A **real decoded GLB** is measured — real geometry, real materials, real textures with real dimensions and a real texel density — which is the claim nothing offline in this repository can make. The pipeline's own report and the budget measurement are taken from that live asset and **must agree exactly**, which is the one-counting-definition claim proved rather than argued. The real streamed voxel world is measured through the real scene graph with a real renderer, twice, identically, and twenty-five measurements change no player, world or scene value. Three seconds of real gameplay call the measurement functions **zero times** — counted live by wrapping them, with three.js's own frame counter proving the game really was rendering. And a real before/after across four seconds of walking produces a structured delta |
...
### Five ways these tests were wrong first, all worth remembering
...
A fifth, from D1 Phase 3, and it is about reading an instrument rather than writing a test:
**`renderer.info.render` describes the LAST RENDER CALL, not the last frame.** This build's
frame ends with the PostFX full-screen quad, so reading it cold reports **1 draw call and 2
triangles** — which looks like a broken measurement and is the correct answer to a
differently-phrased question. Rendering the world explicitly and reading immediately after
gives the world's numbers (367 draw calls, 190,722 of 546,096 resident triangles).
`browser-budgets.js` reports both and says which is which.
